...
def search_baseline_postfilter(query_vector, sql_where_clause, res, data_embed, meta_method='duck', vector_batch_size=100000, k=10):
    ...
    Returns:
        dict: Contains result_indices, filter_time, retrieve_time, result_distances
    """
    print(f"\n--- Running Post-Filtering Query ---")
    print(f"Metadata method: {meta_method}")
    ...
    if meta_method == 'duck':
        try:
            ...
            print(f"Error applying SQL filter with DuckDB: {e}")
        ...
        try:
            ...
            print(f"Error applying indexed filter: {e}")
            print("Please check your filter syntax.")
            return None
    
    else:
        raise ValueError(f"Unknown meta_method: {meta_method}. Use 'duck' or 'index'.")
