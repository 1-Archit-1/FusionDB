...
def search_baseline_prefilter(query_vector, sql_where_clause, res, data_embed, k=10):
    """
    ...
    print(f"2. Vector retrieval (from mmap): Fetched {len(filtered_vectors)} vectors in {retrieve_time:.4f}s")

    start_time = time.time()
    distances, local_indices = brute_force_search(query_vector, filtered_vectors, k=k)
    search_time = time.time() - start_time
    print(f"3. Brute-force search: Completed in {search_time:.4f}s")
    
    original_indices = filtered_ids[local_indices]
